...
class BrnoUrban(Dataset):
    
    def __init__(self, 
                 root : str = '/xdisk/amahalan/ndaba/datasets/BrnoUrban', 
                 mode : str = 'train',
                 aerial_color_space : str = 'RGB',
                 ground_color_space : str = 'RGB'):
        super(BrnoUrban, self).__init__()
        ...
        self.train_list = self.root + 'splits/train-17zl.csv'
        self.test_list = self.root + 'splits/val-17zl.csv'
    ...
    def __getitem__(self, index : int):
        ...
        if self.mode == 'train':
            ...
            ground_image = Image.open(self.root + self.train_id_list[idx][1].rstrip('\n')).\
                convert(self.ground_color_space)
            aerial_image = Image.open(self.root + self.train_id_list[idx][0].rstrip('\n')).\
            ...
            ground_image = Image.open(self.root + self.test_id_list[index][1].rstrip('\n')).\
                convert(self.ground_color_space)
            ground_image = self.transform_ground(ground_image)
            ...
            aerial_image = Image.open(self.root + self.test_id_list[index][0].rstrip('\n')).\
                convert(self.aerial_color_space)
            aerial_image = self.transform_aerial(aerial_image)
